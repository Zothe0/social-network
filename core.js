...
server.get(
    '/model/static/*',
    express.static(path.join(__dirname, 'model', 'static')),
    (req, res) => {
...
server.get(
    '*',
    express.static(path.join(__dirname, 'view', 'build')),
    (req, res) =>
...
async function start() {
    //! Это облачная бд
    const dataBaseURL =
        'mongodb+srv://dev-server:toor@test-db.wl4uh.mongodb.net/server?retryWrites=true&w=majority'
    //! Это бд на моём vps
